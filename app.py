...
# ------- STATION INPUT -------
st.header("Stations (Mainline, Branches, Tap-Off Points)")
station_cols = [
...
# ------- PUMP CURVES -------
st.subheader("Pump Curve Data (Flow vs Head / Efficiency)")
...
for i, stn in enumerate(st.session_state["stations"]):
    if str(stn.get("Is Pump", "")).lower() == "yes":
        ...
        # Flow vs Head Table
        fh_df = st.session_state["curves"].get(f"fh_{i}", pd.DataFrame({"Flow (m3/hr)":[], "Head (m)":[]}))
        fh_edited = st.data_editor(fh_df, num_rows="dynamic", key=f"fh_{i}")
        st.session_state["curves"][f"fh_{i}"] = fh_edited

        # Flow vs Efficiency Table
        fe_df = st.session_state["curves"].get(f"fe_{i}", pd.DataFrame({"Flow (m3/hr)":[], "Efficiency (%)":[]}))
        fe_edited = st.data_editor(fe_df, num_rows="dynamic", key=f"fe_{i}")
        st.session_state["curves"][f"fe_{i}"] = fe_edited

# ------- PEAKS -------
st.subheader("Elevation Peaks (Per Segment)")
if "peaks" not in st.session_state:
    st.session_state["peaks"] = {}
for i, stn in enumerate(st.session_state["stations"]):
    pk_df = st.session_state["peaks"].get(i, pd.DataFrame({"Distance (km) From Upstream":[], "Elevation (m)":[]}))
    pk_edited = st.data_editor(pk_df, num_rows="dynamic", key=f"pk_{i}")
    st.session_state["peaks"][i] = pk_edited

# ------- OPERATING & COST PARAMS -------
...
if st.button("Optimize Pipeline Network"):
    ...
    for i, stn in enumerate(st.session_state["stations"]):
        ...
        # Curve fit
        fh = st.session_state["curves"].get(f"fh_{i}", pd.DataFrame())
        fe = st.session_state["curves"].get(f"fe_{i}", pd.DataFrame())
        head_curve = fh.values.tolist() if not fh.empty else [(0,0),(1,1),(2,2)]
        eff_curve = fe.values.tolist() if not fe.empty else [(0,0),(1,1),(2,2)]
        [A,B,C] = polyfit_head(head_curve)
        [P,Q,R,S,T] = polyfit_eff(eff_curve)
        # Peaks
        peaks_df = st.session_state["peaks"].get(i, pd.DataFrame())
        peaks = [{"loc": float(row[0]), "elev": float(row[1])} for row in peaks_df.values.tolist()] if not peaks_df.empty else []
...
---
**Instructions**
- Use the editable tables to enter station and pump details.
- Use drop-downs for "Is Pump" and "Power Source".
- Enter Flow vs Head and Flow vs Efficiency data for pump stations.
- Peaks can be added per segment in their table.
- Only one of SFC or Rate is required based on power source.
- All labels are Title Case for clarity.
